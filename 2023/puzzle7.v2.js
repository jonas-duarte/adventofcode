...
const input = fs.readFileSync("puzzle7.input", "utf8").split("\r\n");

const cards = [];

const CARDS_ORDER = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"].reverse();
const JOKER = CARDS_ORDER.indexOf("J");

function getType(_hand) {
  const stack = [];

  const hand = [..._hand].sort((a, b) => b - a).filter((c) => c !== JOKER);

  const jokers = 5 - hand.length;

  if (jokers === 5) return 50;
  if (jokers === 4) return 50;
...
for (const line of input) {
  ...
  if(isNaN(power)) throw Error("'power' is not a NaN");
...
console.log(cards.map((c, i) => (i + 1) * c.bid).reduce((a, b) => a + b)); // 249620106
